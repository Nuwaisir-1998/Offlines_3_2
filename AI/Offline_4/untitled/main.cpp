...
string divisor = "1001010";

struct frm global_frame_A;
struct frm global_frame_B;

struct frm frame_to_be_sent_A;
struct frm frame_to_be_sent_B;
...
pair<string, string> CRC(string divident, string divisor, bool che){
    if(crc_steps == 1) {
        cout << "CRC algo:\n";
        cout << "Input : " << divident << "\n";
        cout << "generator polynomial: " << divisor << "\n";
    }
...
//    cout << divident << endl;
    ...
    if(crc_steps == 1) {
        cout << "changed divisor : " << divident << endl;
...
string calculate_checksum_2(struct frm frame){
    ...
    return CRC(ack_bin + seq_bin + type_bin + payload_bin, divisor, false).second;
}

bool check_checksum(string checksum){
    string rem = CRC(checksum, divisor, true).second;
    cout << "rem : " << rem << endl;
    for(auto ele : rem){
...
//    packet_global = packet;
    ...
    if(received_A) {
        struct frm frm_A;
        if(outstanding_ack_A and piggybacking == 1){
            // do piggybacking
            frm_A.type = 2;
            frm_A.acknum = ack_A;
        }else{
...
void B_output(struct pkt packet)
{
    ...
    if(received_B) {
        struct frm frm_B;
        if(outstanding_ack_B and piggybacking == 1){
            // do piggybacking
            frm_B.type = 2;
            frm_B.acknum = ack_B;
        }else{
...
//    bool check = check_checksum(frame.checksum);
    string cs = calculate_checksum_2(frame);
    ...
    if(frame.type == 0){
        if(frame.seqnum == ack_A){
            ...
            if(ack_A == -2){
                ...
                ack_A = frame.seqnum;
            print_frm(frame);
            cout << "------------ tolayer3 called ------------\n";
            tolayer3(0, frame.payload);

            if(piggybacking == 0) {
                frm ack_frame_A;
                ack_frame_A.seqnum = -1;
                ack_frame_A.acknum = ack_A;
                ack_frame_A.type = 1;
                strcpy(ack_frame_A.payload, "ack");
                ack_frame_A.checksum = calculate_checksum_2(ack_frame_A);
                cout << "------------ tolayer1 called ------------\n";
    ...
    if(frame.type == 0){
        if(frame.seqnum == ack_B){
            ...
            if(piggybacking == 0) {
                ...
                ack_frame_B.acknum = ack_B;
                ack_frame_B.type = 1;
                strcpy(ack_frame_B.payload, "ack");
                ack_frame_B.checksum = calculate_checksum_2(ack_frame_B);
                cout << "------------ tolayer1 called ------------\n";
