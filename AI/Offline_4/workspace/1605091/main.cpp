...
public:
    ...
    set<int> blanks;
    int init_blanks_cnt;
    vector<double> map_vertex_impact;
    vector<double> ibh_sum;
    ...
    void init(){
        int rows = board.size();
        map_vertex_impact.resize(rows*rows);
        ibh_sum.resize(rows * rows, 0);
...
//            map_vertex_sdf[ele] = sz;
    ...
    int get_next_ibh(){
        ...
        for(auto ele : blanks){
            Vertex vertex = vertices[ele];
            if(map_vertex_impact[ele] < mn){
                mn = map_vertex_impact[ele];
                idx = ele;
            }
        }
        return idx;
    ...
    int get_next(int heuristic){
        if(heuristic == SDF) return get_next_sdf();
        else if(heuristic == MDH) return get_next_max_dynamic_degree();
        else if(heuristic == IBH) {
            if(ibh_cnt == 0){
                ibh_cnt++;
//                calc_impact();
                for(int i=0;i<map_vertex_impact.size();i++)
                    ibh_sum[i] = map_vertex_impact[i];
            }
            else if(ibh_cnt < 5){
                ibh_cnt++;
                calc_impact();
                for(int i=0;i<map_vertex_impact.size();i++){
                    ibh_sum[i] += map_vertex_impact[i];
                    map_vertex_impact[i] = ibh_sum[i] * 1.0 / ibh_cnt;
                }
            }
            return get_next_ibh();
        }
        else return get_next_no_algo();
    }

    bool backtrack(int s, int heuristic){
        node_cnt++;
    ...
    bool backtrack_fc(int s, int heuristic){
        node_cnt++;
        Vertex s_v = vertices[s];
        assert(s_v.val == 0);
        int cur_row = s_v.pos_x;
        ...
        for(auto i : s_v.dom){
            ...
            if(!impos) {
                if(backtrack_fc(next, heuristic)){
                    return true;
                }else{
                    cnt_backtrack++;
                }
...
//            printst(Dj);
    ...
    void calc_impact(){
        ...
        for(auto blank : blanks){
            ...
            double impact = 0;
            for(auto dom_ele : vertices[blank].dom){
                vector<vector<int>> new_board = board;
                new_board[r][c] = dom_ele;
                Graph g(new_board);
                bool ret = g.AC3();
                double sum = 0;
                if(!ret) {
    ...
    bool backtrack_mac(int s, int heuristic){
//        cout << s << endl;
        node_cnt++;
...
//                }
                next = get_next_sdf();
                if(backtrack_mac(next, heuristic)){
                    return true;
                }else{
                    cnt_backtrack++;
                }
...
//            vertices[s].val = 0;
    ...
    void print(){
        ...
        cout << "Blanks: \n";
        for(auto ele : blanks){
            cout << ele << " " << map_vertex_impact[ele] << endl;
        }
        cout << endl;
        cout << "Doms \n";
        int rows = board.size();
    ...
    void print_info(){
        cout << node_cnt + 1 << " " << cnt_backtrack << endl;
    }

    void run(int backtrack_type, int heuristic){
        auto start_time = high_resolution_clock::now();
        if(backtrack_type == MAC){
            AC3();
        }
        int start = get_next_no_algo();
        if(heuristic == SDF) start = get_next_sdf();
        else if(heuristic == MDH) start = get_next_max_dynamic_degree();
        else if(heuristic == IBH) {
            calc_impact();
            start = get_next_ibh();
        }
        if(backtrack_type == BT){
            backtrack(start, heuristic);
        }else if(backtrack_type == FC){
            backtrack_fc(start, heuristic);
        }else if(backtrack_type == MAC){
            backtrack_mac(start, heuristic);
        }
        auto stop_time = high_resolution_clock::now();
        auto duration = duration_cast<microseconds>(stop_time - start_time);

        cout << node_cnt << " " << cnt_backtrack << " " << duration.count() * 1e-6 << endl;
    }
};
