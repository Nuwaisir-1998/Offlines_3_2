...
int calculate_checksum(struct pkt packet){
    int i, sum = packet.acknum + packet.seqnum;
...
//    message_global = message;
    ...
    if(received) {
        struct pkt pkt_A;
        pkt_A.seqnum = seq_num_A;
        pkt_A.acknum = 0;
        strcpy(pkt_A.payload, message.data);
        pkt_A.checksum = calculate_checksum(pkt_A);
        global_packet = pkt_A;
        printf("A sending: ");
        print_pkt(global_packet);
        tolayer3(0, pkt_A);
        starttimer(0, 10.0);
        received = false;
    }else{
        printf("message: %s dropped\n", message.data);
    }
}
...
void A_input(struct pkt packet)
{
    ...
    if(packet.acknum == seq_num_A){
        int check = calculate_checksum(packet);
...
void B_input(struct pkt packet)
{
    ...
    if(packet.checksum == check){
        ...
        pkt_B.seqnum = 0;
        pkt_B.acknum = ack_B;
        strcpy(pkt_B.payload, "ddd");
        pkt_B.checksum = calculate_checksum(pkt_B);
        printf("B sending: ");
        print_pkt(pkt_B);
        tolayer3(1, pkt_B);
    }else{
        // send inverted received seq
        struct pkt pkt_b_nack;
        pkt_b_nack.seqnum = 0;
        pkt_b_nack.acknum = abs(1 - packet.seqnum);
        strcpy(pkt_b_nack.payload, "nack");
        pkt_b_nack.checksum = calculate_checksum(pkt_b_nack);
        printf("B received\n");
        printf("data distorted : \n");
        print_pkt(packet);
        tolayer3(1, pkt_b_nack);
    }
}
